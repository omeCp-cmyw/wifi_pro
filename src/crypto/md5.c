 ...
 * 函数名称:	get_file_md5
 * 函数描述:	获取文件的MD5值
 * 函数参数:	[in] pFileName:		文件名(带路径)
 * 函数参数:	[in] buf:			存放MD5值缓冲区
 * 函数参数:	[in] bufLen:		缓冲区大小
...
bool get_file_md5(const char *pFileName, unsigned char *buf, unsigned short bufLen)
{
	...
	if (pFileName == NULL || buf == NULL || bufLen < MD5_DIGEST_LENGTH)
	{
		return false;
	}
	...
	memset(buf, 0, bufLen);
	memcpy(buf, md5, MD5_DIGEST_LENGTH);
